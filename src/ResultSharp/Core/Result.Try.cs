...
namespace ResultSharp.Core
{
    public partial class Result
    {
        ...
        public static async Task<Result> TryAsync(Func<Task> func, Func<Exception, Error>? handler = null)
        {
            handler ??= ResultConfigurationGlobal.GetExceptionHandler();
            try
            {
        ...
        public static Result<TResult> Try<TResult>(Func<TResult> func, Func<Exception, Error>? handler = null)
        {
            handler ??= ResultConfigurationGlobal.GetExceptionHandler();
            try
            {
        ...
        /// <param name="handler">The error handler to invoke if an exception occurs.</param>
        /// <returns>A task that represents the asynchronous operation. The task result contains a result indicating success or failure.</returns>
        public static async Task<Result<TResult>> TryAsync<TResult>(Func<Task<TResult>> func, Func<Exception, Error>? handler = null)
        {
            handler ??= ResultConfigurationGlobal.GetExceptionHandler();
            try
            {
